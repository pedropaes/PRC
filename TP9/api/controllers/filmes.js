...
Filmes.getAtores = async function(){
    var query = `select ?idAtor ?anome ?asexo where {
        ?a a c:Ator .
    	?a c:nome ?anome .
    	?a c:sexo ?asexo .
    	bind(strafter(str(?a), 'cinema#') as ?idAtor) .
...
async function getAtorAtomica(idAtor){
    var query = `select ?anome ?asexo where {
        c:${idAtor} a c:Ator .
        c:${idAtor} c:nome ?anome .
        c:${idAtor} c:sexo ?asexo .
    } ` 
    var encoded = encodeURIComponent(prefixes + query)
    try{
        var response = await axios.get(getLink + encoded)
...
Filmes.getFilmesDoAtor = async function(idAtor){
    var query = `select ?idFilme ?titulo where {
        c:${idAtor} c:atuou ?f.
        ?f c:titulo ?titulo .
        bind(strafter(str(?f), 'cinema#) as ?idFilme).
    } 
...
Filmes.getAtor = async function(idAtor){
    try{
        var atomica = await getAtorAtomica(idAtor)
        var filmes = await Filmes.getFilmesDoAtor(idAtor)
